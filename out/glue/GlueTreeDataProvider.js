    ...
    async getChildren(element) {
        if (element) {
            ...
            if (element.TreeItemType === GlueTreeItem_1.TreeItemType.Code) {
                ...
                if (codePath) {
                    result.push(new GlueTreeItem_1.GlueTreeItem(codePath, GlueTreeItem_1.TreeItemType.Detail, element.Region, "", vscode.TreeItemCollapsibleState.None, undefined, element));
                }
                const jobInfo = await GlueTreeView_1.GlueTreeView.Current.getJobInfo(element.ResourceName, element.Region);
                const scriptLocation = jobInfo?.Command?.ScriptLocation;
                if (scriptLocation) {
                    result.push(new GlueTreeItem_1.GlueTreeItem(scriptLocation, GlueTreeItem_1.TreeItemType.Detail, element.Region, "", vscode.TreeItemCollapsibleState.None, undefined, element));
                }
                return result;
            }
            ...
            if (element.TreeItemType === GlueTreeItem_1.TreeItemType.Run) {
                ...
                let children = [];
                // Log nodes
                let outLog = new GlueTreeItem_1.GlueTreeItem("View Output Logs", GlueTreeItem_1.TreeItemType.LogStream, element.Region, run.Id, vscode.TreeItemCollapsibleState.None, undefined, element, { LogGroupName: run.LogGroupName + "/output" });
                outLog.command = { command: 'GlueTreeView.ViewLog', title: 'View Log', arguments: [outLog] };
                children.push(outLog);
                let errLog = new GlueTreeItem_1.GlueTreeItem("View Error Logs", GlueTreeItem_1.TreeItemType.LogStream, element.Region, run.Id, vscode.TreeItemCollapsibleState.None, undefined, element, { LogGroupName: run.ErrorLogGroupName + "/error" });
